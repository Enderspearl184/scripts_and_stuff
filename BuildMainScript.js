...
create.on("activated", (p) => {
	...
	if (p.bricks.length>=p.maxBricks) {
		...
		let placed = brickmodule.newbrick(p, p.buildcore.position, p.brickSize, p.brickrot, p.brickcolor, p.model,p.visibility)
		if (placed) {
			p.bottomPrint("Bricks Placed: " + p.bricks.length + "/" + p.maxBricks)
		}  else {
			p.bottomPrint("You can only build on your plate!")
		}
	}
})
...
function setPlate(p, plate) {
	...
	Game.Plates[plate].open = false
	p.plate = Game.Plates[plate]
}

async function createBuildCore(player) {
	...
	brick = await player.newBrick(brick)
